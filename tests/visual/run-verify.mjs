...
const INTENTIONALLY_REMOVED = {
	...
	// 2026-09-20（66セッション目・段H）: exam の遺伝子の行の直下にあった αの注記
	// #genes-alpha（「遺伝子の読み取りはαテスト中です。まだ作りかけで、結合画像には
	// 反映されません。」）を消した。α という断りは**結合画像にまだ反映していない**
	...
	// 2026-09-21（68セッション目）: ②のパネルの下にあった選択の注記 #deck-selected-note
	// （「✓「◯◯」のN種＋シナリオ因子N種＋遺伝子N種を照合します」）を**丸ごと削除した**
	// （おいもさんの決定）。**この注記に固有の情報が1つも無かった** ―― セット名は直前に
	// 自分で選んだ操作の繰り返し／因子・遺伝子の内訳は②の B・C のチェックがすでに示している／
	// 「N種」は結果の「対象スキル数」カードが出している。
	// **「因子セットを1つ選んでください」（未選択のときの文言）も一緒に消えた** ―― 未選択であることは、
	// ②のタブの件数バッジが出ないこと・OCR のボタンが押せないこと・「②で周回因子セットを選ぶと、
	// 分類の印を付けられるようになります。」（#tier-marks-status）が示す。
	// これで「注記が切り替え直後に画面外へ出る」（D節の68セッション目の項目）も一緒に消えた。
	// `c8fa2d0` に commit したので免除は空に戻した（残すと本当の事故を見逃す口になる）。
	//
	// 2026-09-21（70セッション目・段4）: 選択肢の「続きがある」の合図を、**箱の下端に重なる
	// 半透明の帯**（`data-more-below` ＋ `.usd-opts-wrap::after`）から、**箱の外に出す文字ボタン**
	...
	//
	// 2026-09-23（73セッション目）: 入口の並びを**折り返さず横に送る**形にして1段（30px）に収めたので、
	// **「スキルの追加」の畳む見出し（段11 ⑩）が不要になり、見出しごと消した**
	// ―― `data-usd-act="entry-toggle"` / `data-usd-el="entry-toggle"` / `data-usd-el="entry-body"` と、
	// 入れ物の `.uma-section usd-entry-section` / `.uma-section-body`。**入口の並びは常に見えている。**
...
console.log('\n=== 10. 収録データ（data/）の形 ===');
// data/ の7ファイル（拡張スキル・育成ウマ娘・サポートカード・イベントスキル・シナリオ因子・遺伝子・
// レースの距離の一覧＝2026-09-26・C-97 で追加）は
...
// 実体は tests/catalog/check-catalog.mjs（npm run check:catalog で単体でも回せる）。
{
	...
	if (r.stderr) console.log(r.stderr.trimEnd());
	// 件数は data/ の実ファイルから（§1 と同じ数え方。数を書き写すと、ファイルを足したときに古くなる）
	check(r.status === 0, 'data/ の' + dataFiles.length + 'ファイルが決めた形どおり');
}
