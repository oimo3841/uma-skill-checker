...
async function main() {
	...
	results.forEach((r) => {
		r.substitutions.forEach((s) => {
			...
			const e = byKey.get(k);
			e.per[r.label] = { skillCount: s.skillCount, count: s.count, skills: s.skills };
	...
	const onlyIn = (label) =>
		shown.filter((e) => (e.per[label] ? e.per[label].skillCount : 0) >= minSkills && labels.every((l) => l === label || !e.per[l]));
	...
	lines.push('## 条件');
	lines.push('');
	lines.push('| 条件 | セット | 対象カード | 突き合わせた読み | かけ離れていて外した読み | 読みの条件ごとの完全一致 |');
	lines.push('|---|---|---|---|---|---|');
	results.forEach((r) => {
		const pc = Object.entries(r.perCondition).map(([k, v]) => `${k}: ${v.clean}/${v.reads}`).join('・');
		lines.push(`| ${r.label} | ${r.sets.join(', ')} | ${r.cards} | ${r.usedReads} | ${r.skippedReads} | ${pc} |`);
	});
	lines.push('');
	...
	lines.push('');
	lines.push(`| 候補 | ${labels.join(' | ')} | 合計（種類の和集合） | 既存マップ | 起きたスキル（和集合） |`);
	lines.push(`|---|${labels.map(() => '---').join('|')}|---|---|---|`);
	shown.forEach((e) => {
		lines.push(`| ${e.from}→${e.to} | ${labels.map((l) => cell(e, l)).join(' | ')} | **${e.unionCount}** | ${mapNote(e)} | ${[...e.union].join('・')} |`);
	});
	lines.push('');
	labels.forEach((l) => {
		const list = onlyIn(l);
		lines.push(`## ${l} でしか起きていない読み替え（${minSkills}種以上・${list.length}件）`);
		lines.push('');
	...
	console.log(`\n候補 ${shown.length}件（${minSkills}種以上）`);
	labels.forEach((l) => console.log(`  ${l} でしか起きていない: ${onlyIn(l).length}件`));
