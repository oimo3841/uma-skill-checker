...
- **継ぎ目は「末尾と先頭が丸ごと一致する長さ」では取れない。** 画面の端やバッジに隠れて
  1枚だけ検出できないフレームがあるので、隙間を許した対応づけ（`alignOverlap`）にしてある。
  素の一致だと動画セットで50か所中31か所の継ぎ目が取れず、一覧が161行に割れた。
- **格子の左右が違うカードは、決して同じ一覧の項目ではない。** 片方のフレームで1枚欠けると
  「右の◯◯」と「左の××」が同じ長さで並び、読み違いとして1行にまとめられてしまう
  （実測: 実直な走り／決定打）。対応づけでは左右が違う組を強く罰している。
