...
**画面収録の解像度に注意。** 2026-09-12 の3本目以降の収録は 592x1280（初回の 1180x2556 の半分。
文字の高さ約11px）で届いた。同じ前処理でも完全一致は 71%→28% に落ちる。
2倍に拡大してからOCRしても変わらなかった（実測・同じ17/69枚）。多数決で補えるが、
確認行きは増える。次に撮るときは端末の解像度のまま（圧縮せずに）渡してもらうのがよい。
