# tests/skillset/ — スキルセット画面OCRの検証ハーネス（フェーズ0 → フェーズa）

ゲームの「スキルセット詳細」画面のスクリーンショットから、カードの文字を切り出して
OCRし、445種のマスターに照合する。目的は2つ:

- (a) スキルセットOCR機能の設計材料 → **フェーズa で製品に入った**（`js/skillset-cards.js`・
  `js/skillset-ocr.js`・special.html の「スキルセットのスクショで追加」）。ここからは
  `npm run skillset:product` が製品の実装をそのまま通す回帰テストになる（下記 10.）
- (b) `js/common.js` の文字混同マップ（`CHAR_CONFUSION_MAP`）を底上げする材料

フェーズ0では製品ファイルを一切変更しなかった。フェーズa（コミット1〜5）で切り出しと読み取りの本体を
`lib/` から `js/` へ移し、ハーネスは製品の実体を読む（「仕組み」の節）。**このディレクトリにあるのは
引き続きツールとテストだけ**で、製品からは参照しない。
...
マスター445種は**継承スキルだけ**を対象にしている。システムの仕様上、金スキル・◎（○の強化版）は
継承できないのでマスターに入らない。これらは画面上で**金の地のカード**になるので、
`SkillsetCards.classifyCardKind()` が色で判別し、ハーネスでは **OCRにかけずに「対象外」**にする
（「読めませんでした」に混ぜると、直しようのない警告が毎回出続けるため）。
**進化スキルはこの画面に表示されない**（おいもさんの訂正。以前ここに「進化スキルも金の地」と
書いていたのは誤り）。素材A〜C の実測でも地の色は**ラベンダーと金の2色だけ**で、`unknown` は 0枚
（`skillset-ocr-phase-a-materials-report.md`）。`CARD_KINDS` と `unknown` の構造は将来の色に備えて残す。

製品（`js/skillset-ocr.js`）は金のカードも OCR にかけるが照合はせず、正規化後の完全一致で
**種類数だけ数える**（決定 B-3。「金スキルN種は対象外です」と数だけ出す）。取り込むのはラベンダーだけ。

完全性チェックは役割を分ける（ハーネス。製品は「設定数 N / 200」を撮り漏れの判定には使わず、
数の内訳だけを出す＝決定 A-7）。

- **撮り漏れの判定** … 検出したカードの種類数（ラベンダー＋金の全色）と「設定数 N / 200」を比べる
- **取り込みの結果** … ラベンダーのカードだけをマスターに照合する

ハーネスでは金のカードをOCRにかけないので名前で重複を消せない。画像を撮った順に列として継ぎ合わせ、
