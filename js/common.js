...
const CHAR_CONFUSION_MAP = {
	...
	// 2026-09-13 追加（第2段・54候補／54項目）: 同じ収録で2種のスキルで起きた1文字の誤読のうち、
	// 衝突なし・誤着地なし・見込み1枚以上のもの（rank-confusion-candidates.mjs の第2段）。第1段の上に足した。
	// 採用前後の実測: 一意に一致したが誤り 0 → 0（5,084 → 5,239枚のうち）／確認なしで取り込める 81.4% → 83.9%／
	//   多数決で確定しない行 29 → 23（重なり1枚では 67 → 56）。正規化後の衝突: マスタ445件 0・exam.html 133件 0。
	'姉': '姿', // 2種／延べ87回（攻めの姿勢・気丈な姿勢）
	'忠': '恐', // 2種／延べ105回（恐れぬ心・後先恐れず）
	'勒': '勇', // 2種／延べ32回（勇気の一歩・勇を奮って）
	'妻': '姿', // 2種／延べ16回（攻めの姿勢・気丈な姿勢）
	'根': '極', // 2種／延べ16回（積極策・食の極意）
	'持': '挑', // 2種／延べ25回（夢への挑戦・聖女の挑戦）
	'邂': '避', // 2種／延べ12回（垂れウマ回避・危険回避）
	'寅': '家', // 2種／延べ16回（登山家・自信家）
	'橋': '様', // 2種／延べ41回（おひとり様○・様子見）
	'妥': '姿', // 2種／延べ12回（攻めの姿勢・気丈な姿勢）
	'樟': '様', // 2種／延べ10回（おひとり様○・様子見）
	'桂': '極', // 2種／延べ9回（積極策・食の極意）
	'拘': '抱', // 2種／延べ20回（大志を抱いて・誇りを抱いて）
	'思': '恐', // 2種／延べ11回（恐れぬ心・後先恐れず）
	'邉': '避', // 2種／延べ8回（垂れウマ回避・危険回避）
	'送': '遊', // 2種／延べ17回（遊びはおしまいっ！・泥遊び○）
	'慢': '意', // 2種／延べ15回（外枠得意○・内枠得意○）
	'綺': '線', // 2種／延べ10回（直線回復・直線加速）
	'遮': '進', // 2種／延べ36回（ひたむき前進・前進あるのみ）
	'再': '番', // 2種／延べ26回（一番乗り・ここ一番）
	'盟': '盛', // 2種／延べ19回（盛岡レース場○・盛り返し）
	'ズ': 'ス', // 2種／延べ17回（ポジションセンス・ワンチャンス）
	'毛': '手', // 2種／延べ12回（踏み込み上手・切り替え上手）
	'憎': '惜', // 2種／延べ12回（惜しみなし・労を惜しまず）
	'慶': '離', // 2種／延べ7回（非根幹距離○・超長距離回復○）
	'雀': '復', // 2種／延べ7回（コーナー回復○・超長距離回復○）
	'連': '途', // 2種／延べ7回（前途洋々・夢の途中）
	'密': '家', // 2種／延べ5回（登山家・自信家）
	'荷': '夢', // 2種／延べ5回（夢の再生方法・夢への挑戦）
	'悟': '惜', // 2種／延べ5回（惜しみなし・労を惜しまず）
	'業': '様', // 2種／延べ4回（おひとり様○・様子見）
	'菊': '差', // 2種／延べ4回（差し焦り・差しのコツ○）
	'諾': '髄', // 2種／延べ3回（レースの真髄・賢・レースの真髄・力）
	'酵': '静', // 2種／延べ19回（静かな呼吸・静かな熱）
	'拾': '挑', // 2種／延べ19回（聖女の挑戦・夢への挑戦）
	'益': '盤', // 2種／延べ17回（序盤巧者・中盤巧者）
	'里': '重', // 2種／延べ13回（交流重賞○・地道に重ねて）
	'割': '制', // 2種／延べ13回（先行けん制・追込けん制）
	'磨': '夢', // 2種／延べ11回（夢の再生方法・夢への挑戦）
	'ま': '走', // 2種／延べ11回（真っ直ぐな走り・シビれる走り）
	'榎': '様', // 2種／延べ9回（おひとり様○・様子見）
	'畑': '番', // 2種／延べ7回（一番乗り・ここ一番）
	'効': '勝', // 2種／延べ7回（勝利に向かって・逃げるが勝ち！）
	'溶': '準', // 2種／延べ6回（抜け出し準備・下準備）
	'悦': '意', // 2種／延べ5回（意地っ張り・内枠得意○）
	'毬': '差', // 2種／延べ5回（差しけん制・差し直線○）
	'饅': '差', // 2種／延べ4回（差しコーナー○・差しのコツ○）
	'迪': '迫', // 2種／延べ4回（気迫を込めて・鬼気迫って）
	'価': '備', // 2種／延べ4回（抜け出し準備・外差し準備）
	'防': '開', // 2種／延べ4回（展開窺い・打開策）
	// 2026-09-13 取り消し: 第2段で入れた「'春': '呼'」（2種／延べ4回・静かな呼吸／阿吽の呼吸）を外した。
	// normalizeText は1文字につき1回しか置換しないので、2026-09-06 に入れた「時 → 春」と同居すると
	// 「時」は「春」で止まって「呼」に届かず、鎖が切れる。加えてマスター側の「春ウマ娘○」「春の目覚め」も
	// 「呼ウマ娘○」「呼の直覚め」に寄るため、「春」を「時」と誤読した行（2026-09-06 に対処済みのはずのもの）が
	// 距離0で当たらなくなっていた。2026-09-13 の実機3セットで「春ウマ娘○」の判定漏れとして表面化した。
	// 呼吸系の「呼 → 春」誤読は距離1で拾える（静かな呼吸・阿吽の呼吸とも5文字＝許容距離1）。
	// 教訓: 既に他の項目の行き先になっている字は、新しい項目のキーにしない（ハンドオフメモ F-41）。
	'坪': '差', // 2種／延べ3回（差しためらい・差し駆け引き）
	'昇': '吸', // 2種／延べ3回（深呼吸・阿吽の呼吸）
	'亢': 'ず', // 2種／延べ3回（目にも留まらず・労を惜しまず）
	// 2026-09-13 追加（第3段・7項目）: 2文字のスキル名のための読み替え。2文字以下は allowedDistance() が
	// 距離0（完全一致）を要求するため、1文字の誤読で必ず落ちる。1スキルのための項目だが、
	// 締→綿（連綿）・眼→臨（臨機応変）・淑→泥（泥遊び）と同じ前例に倣った（おいもさんの決定。
	// allowedDistance の緩和は、製品側では距離1の一致が確認なしで入ることと「刀流→力業」の経緯から見送り）。
	// 採用前後の実測: 一意に一致したが誤り 0 → 0（5,239 → 5,261枚のうち）／確認なしで取り込める 83.9% → 84.2%／
	//   多数決で確定しない行 23 → 22（重なり1枚では 56 → 55）。正規化後の衝突: マスタ445件 0・exam.html 133件 0。
	'煌': '煙', // 1種／延べ23回（土煙）
	'揚': '歩', // 1種／延べ11回（巨歩）
	'燈': '煙', // 1種／延べ7回（土煙）
	'燕': '煙', // 1種／延べ3回（土煙）
	...
	'聞': '駆', // 1種／延べ3回（追駆）
	'總': '綿', // 1種／延べ5回（連綿）
	// 2026-09-13 追加（第4段・1項目）: 実機の因子画面（1179x2556・スマホ直撮り3セット）で
	// 「春ウマ娘○」が「晋ウマ嫡○」と誤読され、春／夏／秋／冬の距離1タイが解消できず判定漏れに
	// なっていたパターン（3セット中2セットで同じ誤読。もう1セットには春ウマ娘○が無い）。
	// 「春」の下部の「日」がこのUIフォントで潰れると「晋」に寄る。
	// 誤爆リスクの確認: 「晋」を含むスキルはマスタ445件・exam.html 133件のいずれにも無く、
	// 正規化後の衝突も0（npm run test:norm ／ npm run skillset:check-map）。
	'晋': '春', // 1種／実機2セット（春ウマ娘○）
	// 2026-09-13 追加（第5段・6項目）: 実機の因子画面（1179x2556・スマホ直撮り3セット目）で
	// 「一匹狼」と「船橋レース場○」だけが3回の読み直しすべてで落ちていたパターン。
	//   一匹狼        … 「ー区鶏」（1・2回目）／「ー己猫」（3回目）。3文字は許容距離1なので
	//                    2文字誤読の時点で必ず落ちる。
	//   船橋レース場○ … 「為置レース場○」（1回目）／「韻禰レースズ場○」（3回目）。
	//                    「○○レース場○」の7文字の形が exam.html に14種あり、頭2文字が両方
	//                    崩れると許容距離2で14種全部に並んでタイになる。この画面で既に
	//                    見つかっている8種を外しても6種残るため、位置の手がかりでも絞れない。
	// どちらも頭の1文字が正字へ寄れば距離1の単独首位になり、タイ自体が起きなくなる。
	// 誤爆リスクの確認: 為・区・鶏・猫・韻・禰 を含むスキルはマスタ445件・exam.html 133件の
	// いずれにも無く、正規化後の衝突も0（npm run test:norm ／ skillset:eval-map --exam）。
	// 既存コーパス（20260912_1〜3-full・6,247枚）での実測は前後とも変化なし＝劣化なし。
	// 「置→橋」「己→匹」も同じ誤読で観測したが、置は「位置取り押し上げ」、己は「克己心」
	// 「己を信じて」に実在する字なので、上の1文字で足りる以上は入れない。
	// 「禰」の行き先が「橋」ではなく「様」なのは、既に「橋 → 様」（おひとり様○・様子見）が
	// あり、normalizeText が1文字につき1回しか置換しない＝「船橋レース場○」自身も
	// 「船様レース場○」に正規化されるため。行き先には正規化後の字を書く（ハンドオフメモ F-41）。
	'為': '船', // 1種／実機1セット（船橋レース場○）
	'韻': '船', // 1種／実機1セット（船橋レース場○）
	'禰': '様', // 1種／実機1セット（船橋レース場○・「橋 → 様」の先）
	'区': '匹', // 1種／実機1セット（一匹狼）
	'鶏': '狼', // 1種／実機1セット（一匹狼）
	'猫': '狼', // 1種／実機1セット（一匹狼）
	// 2026-09-13 第6段（30セッション目）: スキルセット画面の「連綿」。切り出しが鮮明でも Tesseract の
	// レイアウト解析（psm 7）が行ごと捨てて何も返さず、skillset-ocr.js の psm 13 での読み直しで
	// 「遣綿」（スマホ原寸 1179px）「運綿」（DMM 986px）「遵錦」（DMM 875px）と距離1〜2で読まれた。
	// 2文字＝許容距離0なので両方の字が正字に寄らないと当たらない。行き先は正規化後の字（「連 → 途」が
	// 既にあるため「連綿」自身は「途綿」に正規化される。「禰 → 様」と同じ理由。F-41）。
	// 誤爆リスクの確認: 遣・運・遵・錦 を含むスキルはマスタ445件・exam.html 133件のいずれにも無く、
	// 正規化後の衝突も0。既存コーパス（20260912_1〜3-full・6,247枚）は前後とも変化なし（skillset:eval-map --exam）。
	// DMM 875px の「遵錦^v」は末尾に「v」が残るので、この読み替えでも当たらない（既知の取りこぼし）。
	'遣': '途', // 1種／実機3セット（連綿・スマホ原寸・psm 13 の読み直し）
	'運': '途', // 1種／実機1セット（連綿・DMM 986px・psm 13 の読み直し）
	'遵': '途', // 1種／実機1セット（連綿・DMM 875px・psm 13 の読み直し）
	'錦': '綿', // 1種／実機1セット（連綿・DMM 875px・psm 13 の読み直し）
	// 2026-09-15 第7段（1項目）: 因子画面の「無三」（2文字＝許容距離0）。実機 test-images/2026-09-15_6
	// （1179x2556・スマホ直撮り）の元画像OCRで全読みが「然三」になり落ちた。結合後OCRでは3変種のうち
	// 1つが「無三」を返して拾えていたが、PC版（2026-09-15_3）でも5読みのうち1つは「然三」で、
	// 結合の有無に関係なく運任せだった（20260913-3 でも同じ誤読を記録済み）。
	// 「無」の上部の横画が太字・丸ゴシックで潰れると「然」に寄る。
	// 誤爆リスクの確認: 「然」を含むスキルは「凛然と」1件のみで、スキル名側にも同じ変換が掛かるので
	// 自身に一致する。正規化後の衝突はマスタ445件・exam.html 133件とも0（npm run test:norm）。
	// 既存コーパス（20260912_1〜3-full・6,247枚）での実測（skillset:eval-map）: 誤着地 0 → 0、
	// 確認に回る行 22 → 21、新たに一致 7行（すべて「然二」→ 無二）。
	'然': '無', // 2種（無二・無三）／実機1セット＋コーパス7行
	// 2026-09-17 第8段（1項目）: 因子画面の「非根幹距離○」の「非」。実機 test-images/2026-09-17_1
	// （765x1360・DMM版）で「菲根幹距難〇」と読まれ、正規化後が「菲極幹距離○」になった。
	// これは「非極幹距離○」（＝非根幹距離○）を含まない一方、短いほうの「極幹距離○」
	// （＝根幹距離○）を部分一致で含むため、matchAllSkills() の部分一致ガードが効かない。
	// あのガードは「同じ行に長短どちらの候補も一致したとき」しか長いほうを残せないので、
	// 頭の「非」が崩れて短いほうだけが一致した行は素通りし、しかも完全一致として
	// rowAssigned が立つのであいまい一致（距離1で正解に着地できたはず）にも回らない。
	// 結果、画面に無い「根幹距離○」を「非根幹距離○」と同時に検出していた（利用者からのFBで発覚）。
	// 元画像・結合後の両方で同じ行から再現する（乱れではなく決定的）。20260913-6 でも同じ
	// 組み合わせを記録済み。「非」の左右の横画が太字・丸ゴシックで潰れると草冠に寄る。
	// 誤爆リスクの確認: 「菲」を含むスキルはマスタ445件・exam.html 133件のいずれにも無いので
	// スキル名側は1件も変わらず、正規化後の衝突も0（npm run test:norm）。
	'菲': '非', // 1種／実機1セット（非根幹距離○）
};
const HOMOGLYPH_MAP = {
	'◯': '○', '〇': '○', '◎': '○', '●': '○', '◉': '○', '0': '○', 'O': '○', 'o': '○', 'Q': '○', 'D': '○', '°': '○',
