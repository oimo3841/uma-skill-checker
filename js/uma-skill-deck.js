...
function renderRecordGrid() {
	...
	/* 「緑スキル」の入口の件数バッジ（72セッション目・段9 の追加とそのやり直し）。数え方は core に借りる
	...
	if (passiveAddedEl) {
		...
		passiveAddedEl.hidden = n === 0;
		passiveAddedEl.textContent = String(n);
	}
	hideNameRevealTip();
	setRecordGridNote('');
