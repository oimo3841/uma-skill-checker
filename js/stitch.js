...
/* **結合画像に焼く「線で描く印」の太さ。** 差し渡し d に比例させ、下限を置く。
   exam の ◇ と special の ◎○△◇ が**これ1つを共有する**（段J-1）。
   段J-1 の前は special だけ `max(1.2, d * 0.08)` を自前で持っていて、
   **2つのツールで線の太さが違っていた**（見え方が食い違っていた原因の1つ）。

   係数 0.16 は段J-1 のモックで決めた（0.10 / 0.13 / 0.16 / 0.20 を実際に描いて比較）。
   0.10 は塗りの ◆ に負け、0.20 は 38px で重くなりすぎる（△ が塊に寄る）。
   **下限 1.4 は 18px では効かない**（0.16 なら 2.88px）。効くのは差し渡し 9px 未満で、
   実際の印はそこまで小さくならないが、極端な画像への保険として残してある。

   **画面の SVG の stroke-width はここから来ていない**（各 HTML の文字列リテラル）。
   同じ数にしても揃わない ―― こちらの d は「元の灰色の〇の直径」で、菱形の幅は
   その 1.4 倍（drawSkillMark の r = d * 0.70）。画面の SVG は viewBox 24 に対して
   菱形の幅が 18 なので、比を合わせるなら別の数になる。段J-1 では**結合画像だけ**を
   変えている（画面の見え方は指摘が出ていないため）。 */
function stitchOutlineWidth(d) {
	return Math.max(1.4, d * 0.16);
}
